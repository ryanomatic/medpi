...
void do_use(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!strcmp(buf, "helm") || !strcmp(buf, "high") || !strcmp(buf, "forest")) {
		if (ch->equipment[WEAR_HEAD] && ch->equipment[WEAR_HEAD]->item_number == 7004) {
			act("$n pauses and looks up at the Helm of the High Forest.", TRUE, ch, NULL, 0, TO_ROOM);
			act("You pause and look up at the Helm of the High Forest.", TRUE, ch, NULL, 0, TO_CHAR);
			cast_farsight(31, ch, buf2, SPELL_TYPE_SPELL, NULL, NULL);
			return;
		}
	}
