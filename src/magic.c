...
void spell_teleport(byte level, struct char_data *ch,
		    struct char_data *victim, struct obj_data *obj)
{
	if (world[ch->in_room] && world[ch->in_room]->zone == 170) {
		send_to_char("Your magic does not seem to work here.\n", ch);
		return;
	}
		   ...
		   struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	case SPELL_TYPE_SCROLL:
		...
		case 'D':
			...
			break;
		default:
			send_to_char("You realize you don't understand that direction.\n", ch);
			return;
		}
		global_color = 33;
		sprintf(log_buf, "$n inhales...folds $s hands across $s chest and looks %s.", direction);
...
void cast_mass_quickness(byte level, struct char_data *ch, char *arg,
		     int type, struct char_data *tar_ch,
		     struct obj_data *tar_obj)
{
	switch (type) {
	case SPELL_TYPE_SCROLL:
		if (tar_obj)
			return;
	case SPELL_TYPE_SPELL:
		for (tar_ch = world[ch->in_room]->people; tar_ch && !IS_NPC(tar_ch); tar_ch = tar_ch->next_in_room)
			spell_quickness(level, ch, tar_ch, 0);
		break;
	default:
		log_hd("Serious screw-up in mass quickness!");
		break;
	}
}

void cast_mass_refresh(byte level, struct char_data *ch, char *arg,
		     int type, struct char_data *tar_ch,
		     struct obj_data *tar_obj)
{
	switch (type) {
	case SPELL_TYPE_SCROLL:
		if (tar_obj)
			return;
	case SPELL_TYPE_SPELL:
		for (tar_ch = world[ch->in_room]->people; tar_ch && tar_ch->master == ch; tar_ch = tar_ch->next_in_room)
			spell_refresh(level, ch, tar_ch, 0);
		break;
	default:
		log_hd("Serious screw-up in mass refresh!");
		break;
	}
}
