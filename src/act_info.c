...
void check_locate(const char* name, const struct obj_data* obj, const struct obj_data* container, int worn, const struct char_data* ch)
{
	char buf[MAX_INPUT_LENGTH] = { 0 };
	char tmp[MAX_INPUT_LENGTH] = { 0 };

	if (isname(name, obj->name)) {
		sprintf(buf, "%sObj [%s%s%s]", COL_BLU, COL_CYN, obj->short_description, COL_BLU);

		if (container) {
			sprintf(tmp, " in container [%s%s%s]", COL_GRN, container->short_description, COL_BLU);
			strcat(buf, tmp);
		}

		if (worn) {
			strcat(buf, " worn as eq");
		}
		else {
			strcat(buf, " in inv");
		}
...
void do_locate(struct char_data *ch, char *argument, int cmd)
{
	...
	sprintf(buf, "%sSearching for items with keyword [%s%s%s]:\n", COL_BLU, COL_YEL, name, COL_BLU);
	send_to_char(buf, ch);

	struct obj_data* obj = NULL;

	for (int eq_slot = 0; eq_slot < MAX_WEAR; eq_slot++) {
		obj = ch->equipment[eq_slot];
		if (obj == NULL)
			continue;

		if (GET_ITEM_TYPE(obj) == ITEM_CONTAINER) {
			for (struct obj_data* item = obj->contains; item; item = item->next_content) {
				check_locate(name, item, obj, TRUE, ch);
			}
		}
		check_locate(name, obj, NULL, TRUE, ch);
	}

	for (obj = ch->carrying; obj; obj = obj->next_content) {
		if (GET_ITEM_TYPE(obj) == ITEM_CONTAINER) {
			for (struct obj_data* item = obj->contains; item; item = item->next_content) {
				check_locate(name, item, obj, FALSE, ch);
			}
		}
		check_locate(name, obj, NULL, FALSE, ch);
	}

	send_to_char(COL_BLU "Done.\n", ch);
	global_color = 0;
}
