...
void do_editroom_exit(struct char_data *ch, char *argument, int cmd)
{
	...
	if (cmd == 9) {
		...
		strcpy(ch->p->queryprompt, "Enter Direction to edit(n,e,s,w,u,d) or cr to cancel > ");
		ch->p->querycommand = 1000;
		return;
	}
	...
	if (cmd == 1000) {
		...
		strcpy(ch->p->queryprompt, "\nEdit what you see when typing LOOK DIRECTION? (y/cr)> ");
		ch->p->querycommand = 1001;
		return;
	}
	...
	if (cmd == 1002) {
		if (keyword[0]) {
			world[ch->in_room]->dir_option[ch->internal_use]->keyword = my_free(world[ch->in_room]->dir_option[ch->internal_use]->keyword);
		...
		strcpy(ch->p->queryprompt, "Enter your selection(0,1,2,3,4,5) or ENTER(cr) for no change> ");
		ch->p->querycommand = 1003;
		return;
	}
	...
	if (cmd == 1005) {
		if (keyword[0]) {
			...
			while (keyword[++number]) {
				...
				world[ch->in_room]->dir_option[ch->internal_use]->entrance = my_free(world[ch->in_room]->dir_option[ch->internal_use]->entrance);
	...
	if (cmd == 1006) {
		if (keyword[0]) {
			...
			while (keyword[++number]) {
				...
				world[ch->in_room]->dir_option[ch->internal_use]->exit = my_free(world[ch->in_room]->dir_option[ch->internal_use]->exit);
...
void do_wiz(struct char_data *ch, char *argument, int cmd)
{
	...
	global_color = (GET_LEVEL(ch) >= 31) ? 36 : 34;
	...
	for (struct descriptor_data* i = descriptor_list; i; i = i->next) {
		if ((!i->connected || i->connected == CON_SOCIAL_ZONE) && GET_LEVEL(i->character) >= mylevel) {
			if (cmd == 5) {
				if (IS_SET(i->character->specials.god_display, GOD_INOUT)) {
					send_to_char(buf1, i->character);
				}
			}
			else if (IS_SET(i->character->specials.god_display, GOD_ONOFF)) {
				send_to_char(buf1, i->character);
			}
		}
	}
